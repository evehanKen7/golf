...
if uploaded_file is not None:
    ...
    if st.button("開始分析"):
        ...
        # OpenCV 先輸出 AVI
        temp_output_avi = tempfile.NamedTemporaryFile(delete=False, suffix=".avi")
        avi_output_path = temp_output_avi.name
        temp_output_avi.close()

        # 轉換後給瀏覽器播放的 MP4
        temp_output_mp4 = tempfile.NamedTemporaryFile(delete=False, suffix=".mp4")
        output_path = temp_output_mp4.name
        temp_output_mp4.close()

        temp_image = tempfile.NamedTemporaryFile(delete=False, suffix=".jpg")
        max_frame_path = temp_image.name
        temp_image.close()

        result = process_video(input_path, avi_output_path, max_frame_path)

        if result is None:
            st.error("影片讀取失敗，請確認檔案格式是否正確。")
        else:
            converted = convert_to_browser_mp4(avi_output_path, output_path)
        ...
        # 清理暫存檔
        for path in [input_path, avi_output_path, output_path, max_frame_path]:
